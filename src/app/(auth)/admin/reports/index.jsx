    ...
    const RenderEmpty = () => (
        <View flex={1}>
          ...
          <YStack flexGrow={1} justifyContent="center" alignItems="center" gap="$3">
            ...
            <Text fontSize="$8" color="$gray9">No open reports!</Text>
          </YStack>
        </View>
      )
            ...
            return (
                <PressableOpacity onPress={() => handlePresentModalPress(item)}>
                    <XStack p="$5" bg="white" justifyContent="space-between" alignItems='center'>
                        <Text fontWeight="$6" allowFontScaling={false}>{msg}</Text>
        ...
        <SafeAreaView flex={1} edges={['left']}>
            ...
            <BottomSheetView style={styles.contentContainer}>
                <XStack justifyContent="space-between" alignItems="center">
                    <Text fontSize="$6" fontWeight="bold">Report #{activeReport?.id}</Text>
                ...
                <XStack mb="$3" justifyContent="space-between" alignItems="center">
                    ...
                    <YStack my="$3" alignItems="center" gap="$3">
                        ...
                        <PressableOpacity onPress={() => gotoProfile(activeReport?.reported_by_account?.id)}>
                            <XStack bg="$gray4" p="$2" gap="$2" alignItems="center" borderRadius={10}>
                                <FastImage
                                    source={{uri: activeReport?.reported_by_account?.avatar}}
                                    style={{width: 20, height: 20, borderRadius: 30}}
                                />
                                <Text fontWeight={'bold'}>{enforceLen(activeReport?.reported_by_account?.acct + 'asdjaskdjaskdjaskdsajk', 15, true)}</Text>
                            </XStack>
                        </PressableOpacity>
                    </YStack>
                </XStack>
                ...
                <Group mt="$5" orientation="vertical" separator={<Separator borderColor="$gray2" />}>
                    <Group.Item>
                        <Button flexGrow={1} size="$5" fontWeight="bold" onPress={() => _handleReport('ignore')}>Ignore</Button>
                    </Group.Item>
                    <Group orientation="horizontal" separator={<Separator vertical borderColor="$gray4" />}>
                        <Group.Item>
                            <Button flexGrow={1} size="$5" fontWeight="bold" onPress={() => _handleReport('unlist')}>Unlist Post</Button>
                        </Group.Item>
                        <Group.Item>
                            <Button flexGrow={1} size="$5" fontWeight="bold" onPress={() => _handleReport('cw')}>Mark Sensitive</Button>
                        </Group.Item>
                    </Group>
                    {/* <Group.Item>
