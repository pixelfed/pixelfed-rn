            ...
            <ScrollView>
                <YStack gap="$3">
                    ...
                    <View marginHorizontal="$3" bg="white" borderRadius={20}>
                        <Group orientation="horizontal" separator={<Separator borderColor="$gray6" vertical />}>
                            <Group.Item>
                                <YStack flexGrow={1} p="$5" justifyContent='center' alignItems='center' gap="$2">
                        ...
                        <XStack p="$5" justifyContent='space-between' alignItems='center' gap="$3">
                            ...
                            <Link href={`/profile/${data?.contact_account?.id}`}>
                                <XStack alignItems='center' gap="$3">
                                    <UserAvatar url={data?.contact_account?.avatar} width={30} height={30} />
                    ...
                    <View marginHorizontal="$3" bg="white" borderRadius={20}>
                        <YStack p="$5">
                            <Text color="$gray9" textAlign='center'>Server Rules</Text>
                            { data?.rules.map((rule, idx) => {
                                return (
                                    <XStack key={idx} py="$3" pr="$3" gap="$3">
                                        <Text fontWeight="bold" color="$gray8">{rule.id}</Text>
                                        <Text>{rule.text}</Text>
                                    </XStack>
                                )
                            })}
                    ...
                    <View marginHorizontal="$3">
                        <Button onPress={() => gotoPrivacyPolicy() } bg="$blue8" size="$5" fontWeight="bold" color="white">Privacy Policy</Button>
                    </View>

                    <View marginHorizontal="$3">
                        <Button onPress={() => gotoTermsOfService() } bg="$blue8" size="$5" fontWeight="bold" color="white">Terms of Service</Button>
                    </View>

                    <View marginHorizontal="$3">
                        <Button onPress={() => gotoHelpCenter() } bg="$blue8" size="$5" fontWeight="bold" color="white">Help Center</Button>
                    </View>
                </YStack>
            </ScrollView>
        </SafeAreaView>
