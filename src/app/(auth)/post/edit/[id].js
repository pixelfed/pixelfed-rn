import { FlatList, ActivityIndicator, StyleSheet, Keyboard, Dimensions } from 'react-native'
import { ScrollView, Separator, Text, View, XStack, YStack, TextArea, Button } from 'tamagui'
...
export default function Page() {
  ...
  const HeaderRight = () => (
    <PressableOpacity onPress={() => _onUpdate()}>
        ...
        <ActivityIndicator />
        :
        <Text fontSize="$5" allowFontScaling={false} fontWeight="bold" color="$blue9">Save</Text>
        }
    </PressableOpacity>
  )
  ...
  const RenderHeader = () => (
    <YStack p="$3" gap="$3">
        ...
        <XStack py="$3" px="$4" bg="white" borderRadius="$3" justifyContent="space-between">
            ...
            <Switch
                size="$3"
                checked={isSensitive}
                onCheckedChange={(checked) => !data?.sensitive ? setSensitive(checked) : null}
            >
        ...
        { isSensitive ? (
            showSpoiler ? 
                <TextArea
                    ...
                    placeholderTextColor={'#ccc'}
                /> :
            <Button size="$5" fontWeight="bold" onPress={() => setShowSpoiler(true)}>Edit CW spoiler text</Button>
        )
        : null }
    </YStack>
  )

  const _updateMediaAlt = useCallback((item, text) => {
    setMedia(media.map(m => {
        if(m.id === item.id) {
            return { ...m, description: text}
        } else {
            return m
        }
    }))
  ...
  const RenderItem = ({item}) => (
    <XStack m="$3" gap="$3">
        ...
        <YStack flexGrow={1} width="70%" gap="$2">
            <Text fontSize="$3" allowFontScaling={false}>Media Alt Text</Text>
            ...
            <TextArea
                ...
                onChangeText={(text) => _updateMediaAlt(item, text)}
                maxLength={1000}
                backgroundColor={'white'}
                numberOfLines={4}
                multiline={true}
            ...
            <XStack justifyContent="flex-end" alignItems="center">
                <Text color="$gray9">{item?.description ? item?.description?.length : '0'}/1000</Text>
            </XStack>
        </YStack>
    </XStack>
  )
                ...
                { isSensitive ? (
                    ...
                    <YStack gap="$1">
                        <XStack justifyContent='space-between' alignItems='center' px="$3">
                            <Text alignSelf='center' allowFontScaling={false} fontWeight={'bold'}>Custom content warning</Text>
                            <PressableOpacity onPress={() => setShowSpoiler(false)}>
                                <Text color="$gray9" allowFontScaling={false}>Hide</Text>
                            </PressableOpacity>
                        </XStack>
                        <TextArea
                            size="$4"
                        ...
                        />
                        <XStack justifyContent="flex-end" alignItems="center">
                            <Text color="$gray9">{spoilerText && spoilerText?.length ? spoilerText?.length : '0'}/140</Text>
                        </XStack>
                    </YStack> :
                    <PressableOpacity onPress={() => setShowSpoiler(true)}>
                        <Text pl="$3" allowFontScaling={false} fontWeight={'bold'}>Edit custom content warning</Text>
                    </PressableOpacity>
                    // <Button chromeless fontWeight="bold" onPress={() => setShowSpoiler(true)}>Edit CW spoiler text</Button>
                )
                : null }
            </YStack>

            <YStack>
                { media.map((item) => (<RenderItem key={`pe-media-${item.id}`} item={item} />)) }
            </YStack>
