...
export default AutolinkText = ({ username, text, onMentionPress, onHashtagPress }) => {
    ...
    return (
        <Text fontSize="$5">
        {username ? <Text fontSize="$5" fontWeight="bold">{username} </Text> : null}
        { parts && parts.map((part, index) => {
            if (part.type === 'mention') {
            return (
                <Text key={index} onPress={() => onMentionPress(`${part.value}`)}  fontSize="$5" color={theme.blue9}>{part.value}</Text>
            );
            ...
            return <Text key={index} fontSize="$5">{part.value}</Text>;
            }
        })}
        </Text>
    );
