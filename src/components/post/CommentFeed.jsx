...
import { FlatList, StyleSheet, Pressable, ActivityIndicator, Dimensions } from 'react-native'
                ...
                <XStack flexShrink={1}>
                    <XStack gap="$3" flexGrow={1}>
                        ...
                        <YStack flexGrow={1} maxWidth={SCREEN_WIDTH - 150} gap={4}>
                            <XStack gap="$2">
                                <Text fontSize="$3" fontWeight="bold">{item.account.acct}</Text>
                                <Text fontSize="$3" color="$gray9">{ _timeAgo(item.created_at) }</Text>
                            </XStack>
                            <Text flexWrap="wrap" fontSize="$4">{ item.content_text }</Text>
                            <Pressable>
                                <Text mt="$2" fontWeight="bold" fontSize="$3" color="$gray9">Reply</Text>
                            </Pressable>
                        </YStack>
                    </XStack>
    ...
    const RenderHeader = useCallback(() => {
        ...
        <YStack mt="$1" mb="$3">
            <XStack pb="$4" justifyContent='center'>
                <Text fontSize="$7" fontWeight={'bold'}>Comments</Text>
            </XStack>
