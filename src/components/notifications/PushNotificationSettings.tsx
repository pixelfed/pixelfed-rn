...
import { pushNotificationSupported, pushState, pushStateDisable, pushStateCompare, pushStateUpdate } from 'src/lib/api';
...
export default function Page() {
  ...
  const mutation = useMutation({
    mutationFn: async (params) => {
      ...
      } else if(params.type === 'notify_like') {
        return await pushStateUpdate({
          notify_enabled: true,
          token: expoPushToken,
          notify_like: params.value
        })
      } else if(params.type === 'notify_follow') {
        return await pushStateUpdate({
          notify_enabled: true,
          token: expoPushToken,
      ...
      } else if(params.type === 'notify_mention') {
        return await pushStateUpdate({
          notify_enabled: true,
          token: expoPushToken,
          notify_mention: params.value
        })
      } else if(params.type === 'notify_comment') {
        return await pushStateUpdate({
          notify_enabled: true,
          token: expoPushToken,
  ...
  const { data: pushStateRes, status, error } = useQuery({ 
    queryKey: ['pushState'], 
    queryFn: pushState,
  })
  ...
  const RenderActivePanel = () => {
    ...
    <ScrollView flexShrink={1}>
        <XStack py="$4" px="$4" bg="white" justifyContent="space-between" alignItems='center'>
            <YStack maxWidth="75%">
                <Text fontSize="$6">
        ...
        <YStack m="$3" bg="white" borderRadius={10} overflow="hidden" zIndex="2">
          <XStack py="$3" px="$4" bg="white" justifyContent="space-between">
              ...
              <Switch 
                ...
                onCheckedChange={(val) => mutation.mutate({type: 'notify_follow', value: val})}>
                  <Switch.Thumb animation="quicker" />
              </Switch>
          </XStack>
          <Separator />
          <XStack py="$3" px="$4" bg="white" justifyContent="space-between">
              ...
              <Switch 
                ...
                onCheckedChange={(val) => mutation.mutate({type: 'notify_like', value: val})}>
                  <Switch.Thumb animation="quicker" />
              </Switch>
          </XStack>
          <Separator />
          <XStack py="$3" px="$4" bg="white" justifyContent="space-between">
              ...
              <Switch 
                ...
                onCheckedChange={(val) => mutation.mutate({type: 'notify_comment', value: val})}>
                  <Switch.Thumb animation="quicker" />
              </Switch>
          </XStack>
          <Separator />
          <XStack py="$3" px="$4" bg="white" justifyContent="space-between">
              ...
              <Switch 
                ...
                onCheckedChange={(val) => mutation.mutate({type: 'notify_mention', value: val})}>
                  <Switch.Thumb animation="quicker" />
              </Switch>
          </XStack>
        </YStack>
